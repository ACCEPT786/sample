<template>
  <el-row>
    <el-col :span="12" :offset="6">
      ...
      <div class="panel panel-primary center-block">
        ...
        <div class="panel-body">
          <el-form>
            <el-form-item label="请先选择:">
              <el-radio-group v-model="form.mcc">
                <el-radio v-for="item in mccOptions" :label="item.value" :key="item.value" style="padding: 5px">{{item.label}}
                </el-radio>
              </el-radio-group>
            </el-form-item>
    ...
    <div style="background-color: slategrey;position: fixed;bottom: 0;width: 100%;text-align: center;height: 6%;">
      <div style="height: 100%;font-size: 20px;line-height: 300%;color: white" @click="handleSystem">系统管理员入口</div>
    </div>
  </el-row>
</template>
<script>
export default {
  ...
  methods: {
    onSubmit() {
      ...
      if (undefined == this.form.mcc || "" == this.form.mcc) {
        this.form.mcc = "460";
      }
      this.$http
        ...
        .then(function(res) {
          ...
          window.sessionStorage.setItem("mcc", this.form.mcc);
          window.sessionStorage.setItem("level", res.data.data.level);
          this.$router.replace({ path: "/panel" });
        });
    },
